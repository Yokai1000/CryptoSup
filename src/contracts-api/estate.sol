...
contract MyLandContract
{
    ...
    function transferLand(address _seller, uint _landID) public payable returns (bool)
    {
        require(msg.sender != _seller, "Can't buy your own house");
        require(msg.value >= __ownedLands[_seller][_landID].price, "The price is not reach, or can't be reach");
        ...
        for(uint i=0; i < (__ownedLands[msg.sender].length);i++)    
        {
            ...
            {
                payable(__ownedLands[msg.sender])
